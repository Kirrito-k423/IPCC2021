	...
	for( int j = 0; j < height; j++ )
	{
		for( int k = 0; k < width; k++ )
		{
			if( 0 > nlabels[oindex] )
			{
				...
				// 	}
				// }
				workq.push(j*width + k);
				saveq.push(j*width + k);
				...
				//-------------------------------------------------------
				if(saveq.size() <= SUPSZ >> 2)
				{
					#pragma omp parallel for
					for( int c = 0; c < saveq.size(); c++ )
					{	
						int qindex;
						// omp_set_lock(&lock);
						qindex = saveq.front();
...
int main (int argc, char **argv)
{
	...
	printf("\nTime taken is %f %f EnforceLabelConnectivity klabels", ftime-itime,ftime-f8time);
