...
void SLIC::RGB2XYZ(
	...
	double&			Z)
{
	double R = sR*0.003921568627451;
	double G = sG*0.003921568627451;
	double B = sB*0.003921568627451;

	double r, g, b;

	if(R <= 0.04045)	r = R*0.0773993808049536;
	else				r = pow((R+0.055)*0.9478672985781991,2.4);
	if(G <= 0.04045)	g = G*0.0773993808049536;
	else				g = pow((G+0.055)*0.9478672985781991,2.4);
	if(B <= 0.04045)
		b = B * 0.0773993808049536;
	else				b = pow((B+0.055)*0.9478672985781991,2.4);
...
void SLIC::RGB2LAB(const int& sR, const int& sG, const int& sB, double& lval, double& aval, double& bval)
{
	...
	double invXr = 1.052126558199433;	//reference white
	double invYr = 1.0;		//reference white
	double invZr = 0.9184811261313391;	//reference white
	...
	double fx, fy, fz;
	if(xr > epsilon)	fx = pow(xr, 0.3333333333333333);
	else				fx = (kappa*xr + 16.0)*0.0086206896551724;
	if(yr > epsilon)	fy = pow(yr, 0.3333333333333333);
	else				fy = (kappa*yr + 16.0)*0.0086206896551724;
	if(zr > epsilon)	fz = pow(zr, 0.3333333333333333);
	else				fz = (kappa*zr + 16.0)*0.0086206896551724;
	...
	double*&					bvec)
{
	...
	lvec = (double *)mmap(NULL,sizeof(double) * sz, PROT_READ | PROT_WRITE,MAP_ANONYMOUS | MAP_PRIVATE, -1,0);
	avec = (double *)mmap(NULL,sizeof(double) * sz, PROT_READ | PROT_WRITE,MAP_ANONYMOUS | MAP_PRIVATE, -1,0);
	bvec = (double *)mmap(NULL,sizeof(double) * sz, PROT_READ | PROT_WRITE,MAP_ANONYMOUS | MAP_PRIVATE, -1,0);
