                ...
                int cb)
{
            ...
            for (int t = 0; t < subgrid[3]; t++) {
                ...
                for (; x + 3 < x_u; x += 4) {
                    ...
                    complex<double> *srcO = src.A +
                        (subgrid[0] * subgrid[1] * subgrid[2] * t + subgrid[0] * subgrid[1] * z + subgrid[0] * y + f_x + (1 - cb) * subgrid_vol_cb) * 12;
                    ...
                    complex<double> *AO = U.A[0] +
                        (subgrid[0] * subgrid[1] * subgrid[2] * t + subgrid[0] * subgrid[1] * z + subgrid[0] * y + x + (1 - cb) * subgrid_vol_cb) * 9;
                    ...
                    */          
                    ...
                    for (int c1 = 0; c1 < 3; c1++) {
                        for (int c2 = 0; c2 < 3; c2++) {
                            vAEReal = _mm256_i32gather_pd(reinterpret_cast<double*>(&AO[c2 * 3 + c1]), AE_vindex, gather_scale);
                            vAEImag = _mm256_i32gather_pd(reinterpret_cast<double*>(&AO[c2 * 3 + c1]) + 1, AE_vindex, gather_scale);
                            vAEReal = _mm256_mul_pd(vAEReal, vNHalf);
                            ...
                            // result now in vtmp = (srcO[0 * 3 + c2] - flag * I * srcO[3 * 3 + c2]) * -half * AE[c1 * 3 + c2]
    ...
    //                 for (int c1 = 0; c1 < 3; c1++) {
    //                     for (int c2 = 0; c2 < 3; c2++) {
    //                         vAOReal = _mm256_i32gather_pd(reinterpret_cast<double*>(&AO[c1 * 3 + c2]), AE_vindex, gather_scale);
    //                         vAOImag = _mm256_i32gather_pd(reinterpret_cast<double*>(&AO[c1 * 3 + c2]) + 1, AE_vindex, gather_scale);
    //                         vAOReal = _mm256_mul_pd(vAOReal, vNHalf); // multiply AE by -0.5: saves some instructions later
    //                         vAOImag = _mm256_sub_pd(vZero, _mm256_mul_pd(vAOImag, vNHalf));
    //                         // vAO = conj(-half * AO[c1 * 3 + c2])
    //                         // don't touch vAO from now on
    ...
    //                         vtmpReal = _mm256_sub_pd(_mm256_mul_pd(vtmp2Real, vAOReal), _mm256_mul_pd(vtmp2Imag, vAOImag));
    //                         vtmpImag = _mm256_add_pd(_mm256_mul_pd(vtmp2Real, vAOImag), _mm256_mul_pd(vtmp2Imag, vAOReal));
    //                         // result now in vtmp = (srcO[0 * 3 + c2] + flag * srcO[2 * 3 + c2]) * conj(-half * AO[c1 * 3 + c2])
    ...
    //                         vtmpReal = _mm256_sub_pd(_mm256_mul_pd(vtmp2Real, vAOReal), _mm256_mul_pd(vtmp2Imag, vAOImag));
    //                         vtmpImag = _mm256_add_pd(_mm256_mul_pd(vtmp2Real, vAOImag), _mm256_mul_pd(vtmp2Imag, vAOReal));
    //                         // result now in vtmp = (srcO[1 * 3 + c2] + flag * srcO[3 * 3 + c2]) * conj(-half * AO[c1 * 3 + c2])
