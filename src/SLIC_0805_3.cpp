...
double itime, ftime, f2time, f3time, f4time,f5time;
	...
	const int&					NUMITR)
{
	...
	while( numitr < NUMITR )
	{
		...
		numitr++;
		//------
		printf("\nmid0 Time %d taken is %f begin",numitr ,omp_get_wtime()-itime);
		...
		if(0 == numitr)
		{
			maxlab.assign(numk,0);
			// maxlab.assign(numk,1);
			// maxxy.assign(numk,1);
		}
		printf("\nmid Time %d taken is %f core",numitr ,omp_get_wtime()-itime);
		...
		for( int i = 0; i < sz-1; i=i+2 )
		{
			...
			maxlab[klabels[i+1]] = max(maxlab[klabels[i+1]],distlab[i+1]);
			// if(maxlab[klabels[i]] < distlab[i]) maxlab[klabels[i]] = distlab[i];
			// if(maxxy[klabels[i]] < distxy[i]) maxxy[klabels[i]] = distxy[i];
		}
		printf("\nmid Time %d taken is %f maxlab",numitr ,omp_get_wtime()-itime);
		//-----------------------------------------------------------------
		...
		printf("\nmid Time %d taken is %f",numitr ,omp_get_wtime()-itime);
		...
		printf("\nmid Time %d taken is %f sigma",numitr ,omp_get_wtime()-itime);
		#pragma omp parallel for
		...
		printf("\nmid Time %d taken is %f inv",numitr ,omp_get_wtime()-itime);
		#pragma omp parallel for
		for( int k = 0; k < numk; k++ )
		{
		...
		printf("\nmid Time %d taken is %f kseed",numitr ,omp_get_wtime()-itime);
	}
...
int main (int argc, char **argv)
{
	...
	ftime = omp_get_wtime();
	printf("\nTime taken is %f", f2time-itime);
	printf("\nTime taken is %f", f4time-itime);
	printf("\nTime taken is %f", f3time-itime);
	printf("\nTime taken is %f", f5time-itime);
