...
void printAVX256d(char * name, __m256d src){
        double saveSrc[4];
	_mm256_store_pd(saveSrc,src);
	printf("%s : \n",name);
	printf("%.2f %.2f %.2f %.2f \n", saveSrc[0], saveSrc[1], saveSrc[2], saveSrc[3]);
}

void printAvxArray(char * name,__m256d * srcReal,char * name2,__m256d * srcImag,int Iter){
	double tmpReal[4],tmpImag[4];
	for(int j=0; j< Iter; j++){
		_mm256_store_pd(tmpReal,srcReal[j]);
		_mm256_store_pd(tmpImag,srcImag[j]);
		printf("%s %s %d: 最后一个不用\n",name,name2,j);
		for (int i=0; i<4; i++){
			printf("%.2f+%.2fi \n", tmpReal[i], tmpImag[i]);
		}
...
int main()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		printAVX256d(tsj(srcStart));
		printAVX256d(tsj(srcStart2));
	...
	printAvxArray(tsj(vSrcReal),tsj(vSrcImag),4);
	...
	printAvxArray(tsj(vTmp1Real),tsj(vTmp1Imag),1);
	printAvxArray(tsj(vTmp2Real),tsj(vTmp2Imag),1);
	...
	printArray(tsj(src0),4,3);
	printArray(tsj(AE),3,3);
	printArray(tsj(destE),4,3);
