                ...
                int cb)
{
    ...
    if (N_sub[0] != 1) {
        ...
        MPI_Send(send_x_b, len_x_f * 6 * 2, MPI_DOUBLE, nodenum_x_b, 8 * rank, MPI_COMM_WORLD);
        MPI_Recv(resv_x_f, len_x_f * 6 * 2, MPI_DOUBLE, nodenum_x_f, 8 * nodenum_x_f,
                 MPI_COMM_WORLD, &sta[8 * nodenum_x_f]);
    }
    ...
    if (N_sub[0] != 1) {
        ...
        MPI_Send(send_x_f, len_x_b * 6 * 2, MPI_DOUBLE, nodenum_x_f, 8 * rank + 1, MPI_COMM_WORLD);
        MPI_Recv(resv_x_b, len_x_b * 6 * 2, MPI_DOUBLE, nodenum_x_b, 8 * nodenum_x_b + 1,
                 MPI_COMM_WORLD, &sta[8 * nodenum_x_b + 1]);
    }
    ...
    if (N_sub[1] != 1) {
        ...
        MPI_Send(send_y_b, len_y_f * 6 * 2, MPI_DOUBLE, nodenum_y_b, 8 * rank + 2, MPI_COMM_WORLD);
        MPI_Recv(resv_y_f, len_y_f * 6 * 2, MPI_DOUBLE, nodenum_y_f, 8 * nodenum_y_f + 2,
                 MPI_COMM_WORLD, &sta[8 * nodenum_y_f + 2]);
    }
    ...
    if (N_sub[1] != 1) {
        ...
        MPI_Send(send_y_f, len_y_b * 6 * 2, MPI_DOUBLE, nodenum_y_f, 8 * rank + 3, MPI_COMM_WORLD);
        MPI_Recv(resv_y_b, len_y_b * 6 * 2, MPI_DOUBLE, nodenum_y_b, 8 * nodenum_y_b + 3,
                 MPI_COMM_WORLD, &sta[8 * nodenum_y_b + 3]);
    }
    ...
    if (N_sub[2] != 1) {
        ...
        MPI_Send(send_z_b, len_z_f * 6 * 2, MPI_DOUBLE, nodenum_z_b, 8 * rank + 4, MPI_COMM_WORLD);
        MPI_Recv(resv_z_f, len_z_f * 6 * 2, MPI_DOUBLE, nodenum_z_f, 8 * nodenum_z_f + 4,
                 MPI_COMM_WORLD, &sta[8 * nodenum_z_f + 4]);
    }
    ...
    if (N_sub[2] != 1) {
        ...
        MPI_Send(send_z_f, len_z_b * 6 * 2, MPI_DOUBLE, nodenum_z_f, 8 * rank + 5, MPI_COMM_WORLD);
        MPI_Recv(resv_z_b, len_z_b * 6 * 2, MPI_DOUBLE, nodenum_z_b, 8 * nodenum_z_b + 5,
                 MPI_COMM_WORLD, &sta[8 * nodenum_z_b + 5]);
    }
    ...
    if (N_sub[3] != 1) {
        ...
        MPI_Send(send_t_b, len_t_f * 6 * 2, MPI_DOUBLE, nodenum_t_b, 8 * rank + 6, MPI_COMM_WORLD);
        MPI_Recv(resv_t_f, len_t_f * 6 * 2, MPI_DOUBLE, nodenum_t_f, 8 * nodenum_t_f + 6,
                 MPI_COMM_WORLD, &sta[8 * nodenum_t_f + 6]);
    }
    ...
    if (N_sub[3] != 1) {
        ...
        MPI_Send(send_t_f, len_t_b * 6 * 2, MPI_DOUBLE, nodenum_t_f, 8 * rank + 7, MPI_COMM_WORLD);
        MPI_Recv(resv_t_b, len_t_b * 6 * 2, MPI_DOUBLE, nodenum_t_b, 8 * nodenum_t_b + 7,
                 MPI_COMM_WORLD, &sta[8 * nodenum_t_b + 7]);
    }
