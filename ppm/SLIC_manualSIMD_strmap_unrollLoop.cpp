	...
	const int&					NUMITR)
{
	...
	f33time = omp_get_wtime();
	// vector<double> distxy(sz, DBL_MAX);
	// vector<double> distlab(sz, DBL_MAX);
	// vector<double> distvec(sz, DBL_MAX);

	double *distxy = (double *)mmap(NULL,sizeof(double) * sz, PROT_READ | PROT_WRITE,MAP_ANONYMOUS | MAP_PRIVATE, -1,0);
	double *distlab = (double *)mmap(NULL,sizeof(double) * sz, PROT_READ | PROT_WRITE,MAP_ANONYMOUS | MAP_PRIVATE, -1,0);
	double *distvec = (double *)mmap(NULL,sizeof(double) * sz, PROT_READ | PROT_WRITE,MAP_ANONYMOUS | MAP_PRIVATE, -1,0);
	...
	while( numitr < NUMITR )
	{
		...
		numitr++;
		//------
		double whiletime = omp_get_wtime();
		...
		for( int n = 0; n < numk; n++ )
		{
			...
			double invMaxlab = 1.0/maxlab[n];
            #pragma omp parallel for 
			for( int y = y1; y < y2; y++ )
			{
				int x = x1;
				long curpos = (long)&distvec[y * m_width + x];
				while((x<x2)&&((curpos>>5<<5!=curpos))){
					int i = y*m_width + x;
					// printf("aligning %ld\n", &distvec[i]);
					// fflush(stdout);
					double l = m_lvec[i];
                ...
                { 
					...
					__m256d vl, va, vb, vx, vy, vl2, va2, vb2, vx2, vy2, vseedl, vseeda, vseedb, vseedx,vseedy,vdistlab, vdistxy, vdist;
					__m256d vl_2, va_2, vb_2, vx_2, vy_2, vl2_2, va2_2, vb2_2, vx2_2, vy2_2, vseedl_2, vseeda_2, vseedb_2, vseedx_2,vseedy_2,vdistlab_2, vdistxy_2, vdist_2;
					__m256d vinvMaxlab, vinvxywt, vdistvec, vn, vklabels;
					__m256d vinvMaxlab_2, vinvxywt_2, vdistvec_2, vn_2, vklabels_2;

					vl = _mm256_loadu_pd(&m_lvec[i]);
					va = _mm256_loadu_pd(&m_avec[i]);
					vb = _mm256_loadu_pd(&m_bvec[i]);
					vdistvec = _mm256_loadu_pd(&distvec[i]);
                    __m128i  * klabelsmemaddrs =(__m128i  *) &klabels[i];
					vklabels = _mm256_cvtepi32_pd(_mm_loadu_si128(klabelsmemaddrs));

					vl_2 = _mm256_loadu_pd(&m_lvec[i+4]);
					va_2 = _mm256_loadu_pd(&m_avec[i+4]);
					vb_2 = _mm256_loadu_pd(&m_bvec[i+4]);
					vdistvec_2 = _mm256_loadu_pd(&distvec[i+4]);
                    __m128i  * klabelsmemaddrs_2 =(__m128i  *) &klabels[i+4];
					vklabels_2 = _mm256_cvtepi32_pd(_mm_loadu_si128(klabelsmemaddrs_2));
					
					vx = _mm256_set_pd((double)(x+3),(double)(x+2),(double)(x+1),(double)(x));
					vx_2 = _mm256_set_pd((double)(x+7),(double)(x+6),(double)(x+5),(double)(x+4));
					vy = _mm256_set1_pd((double)y);
					vn = _mm256_set1_pd((double)n);					
					vseedl = _mm256_set1_pd(kseedsl[n]);
					vseeda = _mm256_set1_pd(kseedsa[n]);
					vseedb = _mm256_set1_pd(kseedsb[n]);
					vseedx = _mm256_set1_pd(kseedsx[n]);
					vseedy = _mm256_set1_pd(kseedsy[n]);
					vinvMaxlab = _mm256_set1_pd((double)invMaxlab);
					vinvxywt = _mm256_set1_pd((double)invxywt);

					vl2 = _mm256_sub_pd(vl, vseedl);
					va2 = _mm256_sub_pd(va, vseeda);
					vb2 = _mm256_sub_pd(vb, vseedb);
					vx2 = _mm256_sub_pd(vx, vseedx);
					vy2 = _mm256_sub_pd(vy, vseedy);
					vl2_2 = _mm256_sub_pd(vl_2, vseedl);
					va2_2 = _mm256_sub_pd(va_2, vseeda);
					vb2_2 = _mm256_sub_pd(vb_2, vseedb);
					vx2_2 = _mm256_sub_pd(vx_2, vseedx);
					vy2_2 = _mm256_sub_pd(vy, vseedy);
					

					vdistlab = _mm256_fmadd_pd(vl2, vl2, _mm256_fmadd_pd(va2, va2, _mm256_mul_pd(vb2, vb2)));
					vdistlab_2 = _mm256_fmadd_pd(vl2_2, vl2_2, _mm256_fmadd_pd(va2_2, va2_2, _mm256_mul_pd(vb2_2, vb2_2)));

					vdistxy = _mm256_fmadd_pd(vx2, vx2, _mm256_mul_pd(vy2, vy2));
					vdistxy_2 = _mm256_fmadd_pd(vx2_2, vx2_2, _mm256_mul_pd(vy2_2, vy2_2));

					vdist = _mm256_fmadd_pd(vdistxy, vinvxywt, _mm256_mul_pd(vdistlab,vinvMaxlab));
					vdist_2 = _mm256_fmadd_pd(vdistxy_2, vinvxywt, _mm256_mul_pd(vdistlab_2,vinvMaxlab));
					
					__m256d vflag,vflag_2;
					vflag = _mm256_cmp_pd(vdist, vdistvec, 1);
					vdistvec = _mm256_blendv_pd(vdistvec, vdist, vflag);
					vklabels = _mm256_blendv_pd(vklabels, vn, vflag);

					vflag_2 = _mm256_cmp_pd(vdist_2, vdistvec_2, 1);
					vdistvec_2 = _mm256_blendv_pd(vdistvec_2, vdist_2, vflag_2);
					vklabels_2 = _mm256_blendv_pd(vklabels_2, vn, vflag_2);

					_mm256_stream_pd(&distvec[i], vdistvec);
					_mm256_stream_pd(&distlab[i], vdistlab);
					_mm_storeu_si128((__m128i  * )&klabels[i], _mm256_cvtpd_epi32(vklabels));


					_mm256_stream_pd(&distvec[i+4], vdistvec_2);
					_mm256_stream_pd(&distlab[i+4], vdistlab_2);
					_mm_storeu_si128((__m128i *)&klabels[i + 4], _mm256_cvtpd_epi32(vklabels_2));
		...
		#pragma omp parallel for reduction(vec_double_max:maxlab) //不能同时加，因为可能会同时写同一个maxlab
		for( int i = 0; i < sz; i++ )
		{
		...
		double maxlabtime = omp_get_wtime();
		//-----------------------------------------------------------------
		...
		double sigmalbeforetime = omp_get_wtime();
		...
		#pragma omp declare reduction(vec_int_plus : std::vector<int> : \
                    ...
                    initializer(omp_priv = decltype(omp_orig)(omp_orig.size()))
		#pragma omp parallel for reduction(vec_double_plus:sigmal,sigmaa,sigmab,sigmax,sigmay) reduction(vec_int_plus:clustersize)
		for( int j = 0; j < sz; j++ )
		{
			int temp = klabels[j];
			//_ASSERT(klabels[j] >= 0);
		...
		double sigmatime = omp_get_wtime();
		...
		double kseedtime = omp_get_wtime();
		// printf("\nmid0 Time %d taken is %f begin",numitr ,omp_get_wtime()-whiletime);
		printf("\nmid Time %d taken is %f core",numitr ,coretime-whiletime);
		printf("\nmid Time %d taken is %f maxlab",numitr ,maxlabtime-coretime);
		printf("\nmid Time %d taken is %f sigmalbeforetime",numitr ,sigmalbeforetime-maxlabtime);
		printf("\nmid Time %d taken is %f sigma",numitr ,sigmatime-sigmalbeforetime);
		printf("\nmid Time %d taken is %f inv",numitr ,invtime-sigmatime);
		printf("\nmid Time %d taken is %f kseed",numitr ,kseedtime-invtime);
	}
}
	...
	const double&				m)//weight given to spatial distance
{
	...
	f5time = omp_get_wtime();
	int* nlabels = new int[sz];
	f6time = omp_get_wtime();
...
int main (int argc, char **argv)
{
	...
	LoadPPM((char *)"homework1.ppm", &img, &width, &height);
	...
	printf("\nTime taken is %f", f1time-itime);
	printf("\nTime taken is %f %f DetectLabEdges", f2time-itime,f2time-f1time);
	printf("\nTime taken is %f %f GetLABXYSeeds_ForGivenK", f3time-itime,f3time-f2time );
	printf("\nTime taken is %f %f PerformSuperpixelSegmentation_VariableSandM 31", f31time-itime,f31time-f3time);
	printf("\nTime taken is %f %f PerformSuperpixelSegmentation_VariableSandM 32", f32time-itime,f32time-f31time);
	printf("\nTime taken is %f %f PerformSuperpixelSegmentation_VariableSandM 33", f33time-itime,f33time-f32time);
	printf("\nTime taken is %f %f PerformSuperpixelSegmentation_VariableSandM", f4time-itime,f4time-f33time);
	printf("\nTime taken is %f %f PerformSuperpixelSegmentation_VariableSandM 循环", f5time-itime,f5time-f4time);
	printf("\nTime taken is %f %f EnforceLabelConnectivity	numlable", f6time-itime,f6time-f5time);
	printf("\nTime taken is %f %f EnforceLabelConnectivity	xvec yvec", f7time-itime,f7time-f6time);
	printf("\nTime taken is %f %f EnforceLabelConnectivity iteration", f8time-itime,f8time-f7time);
	printf("\nTime taken is %f %f EnforceLabelConnectivity klabels\n", ftime-itime,ftime-f8time);
	// printf("\nTime taken is %f", f6time-itime);
	// printf("\nTime taken is %f", ftime-itime);
	fflush(stdout);
	sleep(5);
	auto compTime = chrono::duration_cast<chrono::microseconds>(endTime - startTime);
	cout << "Computing time=" << compTime.count() / 1000 << " ms" << endl;

	int num = CheckLabelswithPPM((char *)"check_test.ppm", labels, width, height);
	if (num < 0) {
	...
	slic.SaveSuperpixelLabels2PPM((char *)"output_labels_test.ppm", labels, width, height);
